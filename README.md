...
* 当前稳定分支已保留 PVE 地图与 PVP/UI 美术整合改动，PVE HUD 回到覆盖式完整标签显示，避免与底图内置文字叠加。
* PVP 目前包含 `pvp_sunny_beach` 与 `pvp_office_panic` 两套硬编码布局，路径逻辑集中在 `src/ui/PvpMapLayout.h`，尚未恢复到 JSON 配置加载管线。
* PVP 的运行时路径覆盖层已移除；如果需要玩家明确看到 Beach 路径，应优先制作带路径的正式底图资源，而不是在 UI 运行时叠加色块。
* 卡牌数值已统一到共享规格表，图鉴展示与战斗实例化应共用同一来源。
* 本地验证命令：`cmake --build build-latest`。
