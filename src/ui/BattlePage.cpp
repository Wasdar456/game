...
void BattleView::drawProjectiles(QPainter &painter)
{
    for (const auto &projectile : m_snapshot.projectiles) {
        ...
        const double startX = projectile.fromCol * CELL_SIZE + CELL_SIZE / 2.0;
        const double startY = projectile.fromRow * CELL_SIZE + CELL_SIZE / 2.0;
        const double endX = projectile.toCol * CELL_SIZE + CELL_SIZE / 2.0;
        const double endY = projectile.toRow * CELL_SIZE + CELL_SIZE / 2.0;
        ...
        if (projectile.kind == game::core::ProjectileKind::Aoe && projectile.splashRadius > 0) {
            const double splashPixels = projectile.splashRadius * CELL_SIZE;
...
QByteArray BattlePage::serializeBattleState(const game::core::BattleSnapshot& snapshot) const
{
    QByteArray body;
    ...
    out << static_cast<quint32>(battleStateChecksum(snapshot));

    return body;
}

bool BattlePage::parseBattleState(const QByteArray& body, game::core::BattleSnapshot& snapshot) const
{
    QDataStream in(body);
    ...
    quint16 unitCount = 0;
    in >> currentWave >> waveActive >> hostResources >> hostBaseHealth >> clientResources >> clientBaseHealth >> unitCount;
    if (in.status() != QDataStream::Ok) return false;

    snapshot = game::core::BattleSnapshot();
    ...
    snapshot.map = m_battleManager ? m_battleManager->snapshot().map : game::core::MapSnapshot();
    ...
    if (!in.atEnd()) {
        ...
        in >> remoteChecksum;
        game::core::BattleSnapshot hostPerspective = snapshot;
        hostPerspective.resources = snapshot.opponentResources;
        hostPerspective.baseHealth = snapshot.opponentBaseHealth;
        hostPerspective.opponentResources = snapshot.resources;
        hostPerspective.opponentBaseHealth = snapshot.baseHealth;
        const quint32 localChecksum = battleStateChecksum(hostPerspective);
        if (in.status() == QDataStream::Ok && remoteChecksum != localChecksum) {
            qDebug() << "[BattlePage] BATTLE_STATE checksum mismatch, remote:"
                     << remoteChecksum << "local:" << localChecksum;
        }
    }
    return true;
}

quint32 BattlePage::battleStateChecksum(const game::core::BattleSnapshot& snapshot) const
{
...
void BattlePage::onNetworkPacket(game::network::MsgType type, const QByteArray& body)
{
    ...
    case game::network::MsgType::BATTLE_STATE: {
        if (!m_inBattlePhase || m_isHost) break;
        game::core::BattleSnapshot remoteSnapshot;
        if (parseBattleState(body, remoteSnapshot)) {
            handleRemoteBattleState(remoteSnapshot);
        } else {
